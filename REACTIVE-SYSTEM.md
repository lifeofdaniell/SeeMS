...
### Bug 1: Images Not Loading (403 Forbidden)

**Problem:** Strapi returns 403 Forbidden for unauthenticated API requests
**Fix:** Enable public read permissions via Strapi bootstrap file

**Bootstrap File:** `strapi-bootstrap/index.ts` (auto-generated by converter)
- Auto-enables `find` and `findOne` permissions for all content types
- Runs on Strapi startup
- Copy to your Strapi project at `src/index.ts`

**Manual Alternative:** Settings → Users & Permissions → Roles → Public → Check `find` and `findOne`

### Bug 2: Image URLs Not Populated

**Problem:** Strapi v5 doesn't populate media fields by default
**Fix:** Add `populate: '*'` query parameter and transform image objects to URLs

**Before:**
```typescript
useFetch(`${strapiUrl}/api/${pageName}`)
// Returns: { hero_image: null } ❌
```

**After:**
```typescript
useFetch(`${strapiUrl}/api/${pageName}`, {
  query: { populate: '*' } // Strapi v5 populate parameter
})
// Returns: { hero_image: { url: '/uploads/...', mime: '...', ... } }
// Transformed to: { hero_image: 'http://localhost:1337/uploads/...' } ✅
```

**Transformation Logic:**
- Detects Strapi media objects (has `url` + `mime`/`formats`)
- Extracts URL and prepends Strapi base URL
- Handles arrays of images (collections)
- Recursively transforms nested objects

### Bug 3: Preview Mode Shows Blank Page
